...
const handleFileUpload = (e) => {
    ...
    readWorkbook(file, XLSX, (workbook) => {
      ...
      console.log("📑 Resultado de processSheets():", result);
      ...
      console.log("📑 Claves en sheets:", Object.keys(sheets)); // Ver qué claves tiene

      // Obtener el array correcto (puede ser "Plantilla" u otro)
      const sheetKey = Object.keys(sheets)[0]; // Tomar la primera clave
      const sheetData = sheets[sheetKey];

      if (!Array.isArray(sheetData)) {
        console.error("❌ Error: sheets['" + sheetKey + "'] no es un array.", sheetData);
        return;
      }
      ...
      console.log("📊 Datos que se pasan a Spreadsheet:", formattedData);
    });
  };
*/